...
import config
import utils
from mail_notify import notify_task_updated
...
def today_page():
    ...
    def in_exec_dates(d):
        """今天是否在執行日期清單內"""
        raw = d.get('執行日期', '').strip()
        if not raw:
            return False
        return today in [s.strip() for s in raw.split(',') if s.strip()]
    ...
    return jsonify({
        "today": [d for d in simple if in_exec_dates(d)],
...
def add_task():
    data     = utils.load_data()
    req      = request.json
    if not (req.get('項目Due Date') or '').strip():
        return jsonify({"status": "error", "message": "項目Due Date 為必填"}), 400
...
def update_task():
    ...
    updatable = ['管理OWNER', '項目Due Date', '項目OWNER', '單項目Due Date',
                 '項目描述', '案件分類', '組織類別', '棟別', '樓層', '站點', '狀態', '執行日期', 'Done_Day']
    changed = {f: req[f] for f in updatable if f in req and req[f] != task.get(f, '')}
    old_status = task.get('狀態', '')
    ...
    # ── 寄信通知（僅狀態變動時）──
    if '狀態' in changed:
        notify_task_updated(task, changed, account, old_status)
