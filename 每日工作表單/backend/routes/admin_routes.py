...
def get_admin_config_for(account):
    ...
    # 只允許這四個固定組織顯示於前端，其餘 admin.json 內容（如 FT01_資 等）一律忽略
    VALID_ORGS = ['FT01營運(硬)', 'FT01營運(資)', 'FT01營運(保)', 'FT01值班']
    full = utils.load_admin_config()
    cfg  = {org: full.get(org, []) for org in VALID_ORGS if org in my_orgs}
...
def update_admin_config(account):
    ...
    if org not in cfg or org not in VALID_ORGS:
    ...
    # 回傳時也只保留四個固定組織，避免 FT01_資 等內部 key 洩漏到前端
    scoped_cfg = {o: cfg.get(o, []) for o in VALID_ORGS if o in my_orgs}
    return jsonify({"status": "ok", "config": scoped_cfg})