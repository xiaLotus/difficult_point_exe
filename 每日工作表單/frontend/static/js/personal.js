...
const app = Vue.createApp({
    ...
    methods: {
        ...
        openAddModal() {
            ...
            this.formError=''; this.newExecDate=''; this.showAddModal=true
        },
        async submitAdd() {
            if (!this.addForm['日期'])           { this.formError='請填寫日期'; return }
            if (!this.addForm['項目描述']?.trim()) { this.formError='請填寫項目描述'; return }
            if (!this.addForm['項目Due Date'])   { this.formError='請填寫項目Due Date'; return }
        ...
        // ── 執行日期管理 ──
        execDates() {
            const raw = (this.addForm['執行日期'] || '').trim()
            return raw ? raw.split(',').map(s => s.trim()).filter(Boolean) : []
        },
        addExecDate(dateStr) {
            if (!dateStr) return
            const dates = this.execDates()
            if (!dates.includes(dateStr)) {
                dates.push(dateStr)
                dates.sort()
                this.addForm['執行日期'] = dates.join(',')
            }
            this.newExecDate = ''
        },
        removeExecDate(dateStr) {
            const dates = this.execDates().filter(d => d !== dateStr)
            this.addForm['執行日期'] = dates.join(',')
        },
