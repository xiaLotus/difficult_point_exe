...
Vue.createApp({
    ...
    watch: {
        ...
        'editForm.狀態'(val) {
            if (val === 'Done' && !this.editForm['Done_Day']) {
                const now = new Date()
                const y = now.getFullYear(), m = String(now.getMonth()+1).padStart(2,'0'), d = String(now.getDate()).padStart(2,'0')
                this.editForm['Done_Day'] = `${y}-${m}-${d}`
            }
        },
    },
    ...
    methods: {
        ...
        // ── 執行日期管理 ──
        execDates() {
            const raw = (this.editForm['執行日期'] || '').trim()
        ...
        try {
            ...
            if (this.task) {
                this.editForm = {
                    ...
                    '狀態':          this.task['狀態']         || 'On Going',
                    '執行日期':      this.task['執行日期']     || '',
                    'Done_Day':      this.task['Done_Day']     || '',
                }
            }
